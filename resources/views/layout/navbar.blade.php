    ...
    <body class="font-sans antialiased">
        ...
        <nav id="nav" class="w-nav">
            ...
            <ul class="nav-list" id="menuList">
                <!--welcome --->
                <li class="s">
                    <a href="{{ url('/') }}" class="menu-item">{{ translate('Acceuil') }}</a>
                </li>
                {{-- products --}}
                ...
                {{-- contact --}}
                <li><a href="{{ route('contact') }}">{{ translate('Contact') }}</a></li>
                <li class="mob-lang">
                    <div class="col-md-4 w-100">
                        <select
                            ...
                            <option style="background-color:#000099">{{ translate('Sélectionner la langue') }}</option>
                            <option style="background-color:#000099" value="fr" {{ session()->get('locale') == 'fr' || !session()->has('locale') ? 'selected' : '' }}>Français</option>
                            <option style="background-color:#000099" value="en" {{ session()->get('locale') == 'en' ? 'selected' : '' }}>English</option>
                            <option style="background-color:#000099" value="ar" {{ session()->get('locale') == 'ar' ? 'selected' : '' }}>Arabe</option>
                        </select>
                    </div>
                </li>
            </ul>
            ...
            <select id="languageSelect" style="color:#ffd700; cursor:pointer; padding-bottom:10px; background-color: rgba(255, 255, 255, 0); border:none; font-size:20px;" class="form-select pe-5 changeLanguage" aria-label="Language select">
                    <option class="changeoption"style="background-color:#000099;">{{ translate('Sélectionner la langue') }}</option>
                    <option style="background-color:#000099;" class="changeoption" value="fr" {{ session()->get('locale') == 'fr' || !session()->has('locale') ? 'selected' : '' }}>Français</option>
                    <option style="background-color:#000099;" class="changeoption"  value="en" {{ session()->get('locale') == 'en' ? 'selected' : '' }}>English</option>
                    <option style="background-color:#000099;" class="changeoption" value="ar" {{ session()->get('locale') == 'ar' ? 'selected' : '' }}> Arabe</option>
            </select>
            </div>
        </div>
            ...
            <div class="container p-4 pb-0">
                <div class="row">
                    ...
                    <div class="col-md-3 col-lg-3 col-xl-3 mx-auto mt-3">
                        <h6 class="text-uppercase mb-4 font-weight-bold">{{ translate('À propos') }}</h6>
                        <p>{{ translate(" Depuis sa création en 2000, EVO machinery s'est rapidement imposé comme l'un des leaders mondiaux du secteur des équipements de construction et des engins de levage.") }}</p>
                    </div>
                    <hr class="w-100 clearfix d-md-none"/>
                    <!-- Products -->
                    <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mt-3">
                        <h6 class="text-uppercase mb-4 font-weight-bold">{{ translate('Produits')}}</h6>
                            @foreach ($categories->take(5) as $categorie)
                                <p><a class="text-white" style="text-decoration: none" href="{{ url('product/' . $categorie->id) }}">{{ translate($categorie->nom_cat) }}</a></p>
                            @endforeach
                            <p><a class="text-white" style="text-decoration: none" href="{{ route('category.liste') }}"> {{ translate('voir tous') }} {{ ' ' }}<i class="fa fa-arrow-right"></i></a></p>
                </div>
                <hr class="w-100 clearfix d-md-none" />
                <!-- Contact Us -->
                <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-3">
                    <h6 class="mb-4 font-weight-bold">{{ translate('CONTACTEZ-NOUS') }}</h6>
                    <p>
                        <i class="fas fa-home mr-3"></i>
                        <a href="https://www.google.com/maps/search/HOLLYWOOD+MONGKOK+,+KOWLOON+,+HONG+KONG/@22.317164,114.170363,20z?hl=fr-FR&entry=ttu&g_ep=EgoyMDI1MDEyMi4wIKXMDSoASAFQAw%3D%3D" style="text-decoration: none; color: inherit;" target="_blank">HOLLYWOOD MONGKOK , KOWLOON , HONG KONG</a>
                    </p>
                    <p>
                        <i class="fas fa-envelope mr-3"></i>
                        <a href="mailto:contact@evo-machinery.com"style="color:white;text-decoration: none">contact@evo-machinery.com</a>
                    </p>
                    <p>
                        <i class="fas fa-envelope mr-3"></i>
                        <a href="mailto:sales@evo-machinery.com"style="color:white;text-decoration: none">sales@evo-machinery.com</a>
                    </p>
                    <p>
                ...
                <div style="width: 300px !important" class="mx-auto mt-3 col-md-2 col-lg-2 col-xl-2">
                    <h6 class="mb-4 text-uppercase font-weight-bold">{{ translate('Recherche') }} </h6>
                    <div class="search-bar col-12 text-center mt-4 mb-4">
                        <form action="{{ route('category.search') }}" method="GET" class="d-inline-flex">
                            <input type="text" class="form-control form-control-lg search-input" name="query" placeholder="modèle de produit.." aria-label="Search" style=" border: 1px solid #ccc; padding: 10px;">
            ...
            <div class="p-3 pt-0">
                <div class="row d-flex align-items-center">
                    ...
                    <div class="col-md-7 col-lg-8 text-center text-md-start">
                        <div class="p-3">© 2025 Copyright: <a class="text-white" href="https://evo-machinery.com">evo-machinery.com</a></div>
                    </div>
                </div>
            </div>
            </div>
